...
exports.createPost = async (req, res) => {
    try {
        ...
        // Bulk-create notifications for all other users using insertMany
        const allOtherUsers = await User.find({ _id: { $ne: req.user.id } }).select('_id');
        console.log(`New post created. Notifying ${allOtherUsers.length} other users.`);

        if (allOtherUsers.length > 0) {
            const notifications = allOtherUsers.map(user => ({
                recipient: user._id,
                sender: req.user.id,
                type: 'new_post',
                post: post._id,
            ...
            if (req.io && req.userSockets) {
                for (const notification of savedNotifications) {
                    const recipientSocketId = req.userSockets.get(notification.recipient.toString());
                    if (recipientSocketId) {
                        ...
                        req.io.to(recipientSocketId).emit('new_notification', populated);
                    }
                }
            }
        }
